DELIMITER //
DROP PROCEDURE IF EXISTS spBillingGenerateTermBilling2017//
CREATE PROCEDURE spBillingGenerateTermBilling2017(paramTerm int(11), paramCreatedBy varchar(45))
BEGIN
				  ...
				  from billingStudent bsSub
					...
					where bsi.includeFlag = 1 and bsSub.includeFlag = 1
						...
						and bsSub.term in (select term from bannerCalendar
											where ProgramYear = (select ProgramYear from bannerCalendar where term = paramTerm)
											)
	...
	FROM (SELECT billingStudent.contactId, billingStudent.billingStudentId, firstname, lastname, billingStudent.bannerGNumber,
		...
		left outer join
			(SELECT contactId, Program, DistrictID,
					sum(COALESCE(PostBillCorrectedBilledAmount, FinalBilledAmount, CorrectedBilledAmount, GeneratedBilledAmount)) Days
			FROM billingStudent
			WHERE includeFlag = 1 
	...
	GROUP BY bs.firstname, bs.lastname, bs.enrolledDate, bs.bannergnumber
			,bs.Program
			,schooldistrict.schoolDistrict
			,bsOther.Days;
