...
BEGIN
	...
	FROM stmp_EnrollExit data
		LEFT OUTER JOIN sptmp_EnrollExitCopy1 dataNext 
			ON data.contactId = dataNext.contactId
				AND data.ID + 1 = dataNext.ID
		LEFT OUTER JOIN sptmp_EnrollExitCopy2 dataPrev 
			ON data.contactId = dataPrev.contactId
   ...
   #add to currentEnrollExit
    ...
    SELECT contactId, max(ID) ID, min(enrolledDate) minEnrolledDate, max(exitDate) maxExitDate
        ...
        ,sum(IFNULL(postGED_GPACredits,0)) postGED_GPACredits
        ,sum(IFNULL(postGED_GPAPoints,0)) postGED_GPAPoints
   ...
   #get latest exit entries per contactid
	...
	UPDATE sptmp_CurrentEnrollExitRevised
        JOIN
			...
			GROUP BY contactId
				...
				ON sptmp_CurrentEnrollExitRevised.contactId = data.contactID 
	SET 
	...
	UPDATE reporting.contactProfile
		LEFT JOIN sidny.bannerCalendar ON GEDMaxDate >= bannerCalendar.TermBeginDate
